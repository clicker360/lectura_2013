...
class rc_sweet_custom_menu {
	...
	function __construct() {
		...
		// add custom menu fields to menu
		add_filter( 'wp_setup_nav_menu_item', array( $this, 'rc_scm_add_custom_nav_fields_color' ) );

		// save menu custom fields
		add_action( 'wp_update_nav_menu_item', array( $this, 'rc_scm_update_custom_nav_fields'), 10, 3 );

		add_action( 'wp_update_nav_menu_item', array( $this, 'rc_scm_update_custom_nav_fields_color'), 10, 3 );
	...
	*/
	...
	function rc_scm_add_custom_nav_fields_color( $menu_item ) {
	...
	*/
	...
	function rc_scm_update_custom_nav_fields_color( $menu_id, $menu_item_db_id, $args ) {
	
	    // Check if element is properly sent
	    if ( is_array( $_REQUEST['menu-item-color-text']) ) {
	        $color_value = $_REQUEST['menu-item-color-text'][$menu_item_db_id];
...
/** Agregar el selector de color **/
function colorpicker_enqueue() {
    wp_enqueue_style( 'wp-color-picker' );
    wp_enqueue_script( 'colorpicker-js',  plugins_url() . '/sweet-custom-menu/js/colorpicker.js', array( 'wp-color-picker' ) );
}
